...
for mov in movies:
    movie = Movie()
    movie.director = Director.objects.get(name = mov[0])
    movie.genre = Genre.objects.get(title = mov[1])
    movie.title = mov[2]
    movie.opening_date = mov[3]
    movie.running_time = mov[4]
    movie.screening = mov[5]
    movie.save()
```
...
```python
for dic in Movie.objects.all():
    print(dic.director, dic.genre, dic.title, dic.opening_date, dic.running_time, dic.screening)
```
...
```python
for dic in Movie.objects.all():
    print(dic.director.name, dic.genre, dic.title, dic.opening_date, dic.running_time, dic.screening)
```
...
```python
for dic in Movie.objects.all():
    print(dic.director.name, dic.genre.title, dic.title, dic.opening_date, dic.running_time, dic.screening)
```
...
```python
for dic in Movie.objects.order_by("-opening_date"):
    print(dic.director.name, dic.genre.title, dic.title, dic.opening_date, dic.running_time, dic.screening)
```
...
```python
dic = Movie.objects.order_by("opening_date")[0]
...
```python
for dic in Movie.objects.filter(screening = True).order_by("opening_date"):
    print(dic.director.name, dic.genre.title, dic.title, dic.opening_date, dic.running_time, dic.screening)
```
...
```python
director_ = Director.objects.filter(name = "봉준호")
movie_ = Movie.objects.filter(director_id__in=director_).order_by("opening_date")
...
```python
director_ = Director.objects.filter(name = "봉준호")
dic = Movie.objects.filter(director_id__in=director_).order_by("opening_date")[1]
...
```python
movie_ = Movie.objects.filter(running_time__gt = 119).order_by("running_time")
...
```python
movie_ = Movie.objects.filter(running_time__gte = 119).order_by("running_time")
...
```python
movie_ = Movie.objects.filter(opening_date__gt = "2022-01-01").order_by("opening_date")
...
genre_ = Genre.objects.filter(title = "드라마")
movie_ = Movie.objects.filter(director_id__in = director_, genre_id__in = genre_).order_by("opening_date")
...
```python
director_ = Director.objects.filter(name = "봉준호")
movie_ = Movie.objects.exclude(director_id__in = director).order_by("opening_date")
